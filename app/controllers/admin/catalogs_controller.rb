class Admin::CatalogsController < ApplicationController
  ...
  # GET /catalogs/1.json
  def show
    @catalog = Admin::Catalog.find(params[:id])
  ...
  # POST /catalogs.json
  def create
    @catalog = Admin::Catalog.new(params[:admin_catalog])
  ...
  def update
    ...
    respond_to do |format|
      if @catalog.update_attributes(params[:admin_catalog])
